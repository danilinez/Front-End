...
public partial class ControlPanel_ManageCases : ControlPanelSessionManager
{
    ...
    private void fnLoadCases()
    {
        ...
        if (txtCaseRef.Text != "")
        {
            if (!isWhereExists)
            {
                strQuery += " where t1.sCaseRefNo Like '%" + txtCaseRef.Text + "%'";      //WHCS-01
                isWhereExists = true;
            }
            else
        ...
        if (txtDocName.Text != "")
        {
            if (!isWhereExists)
            {
                strQuery += " where  t1.sDoctorName Like '%" + txtDocName.Text + "%'";                  //Dr. Akbar
                isWhereExists = true;
            }
            else
        ...
        if (txtCaseDate.Text != "")
        {
            if (!isWhereExists)
            {
                strQuery += " where  cast(t1.dtCaseDate as date)= Cast('" + txtCaseDate.Text + "' as date)";       //27 Mar 2013
                isWhereExists = true;
            }
            else
        ...
        if (txtLocationName.Text != "")
        {
            if (!isWhereExists)
            {
                strQuery += " where  t1.sLocationName Like '%" + txtLocationName.Text + "%'";    // 75 The Mall
                isWhereExists = true;
            }
            else
    ...
    protected void lstCase_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        ...
        {
            Users objQuery = new Users();
            objQuery.ExecuteSql("delete from PrescriptionDetails where exists (select * from Prescriptions where iPrescriptionID = PrescriptionDetails.iPrescriptionID and iCaseID = " + e.CommandArgument.ToString() + ")");
            objQuery.ExecuteSql("delete from Prescriptions where iCaseID = " + e.CommandArgument.ToString() + "");
            objQuery.ExecuteSql("delete from GeneralQA where iCaseID = " + e.CommandArgument.ToString() + "");
            objQuery.ExecuteSql("delete from PaymentInformation where iCaseID = " + e.CommandArgument.ToString() + "");
            objQuery.ExecuteSql("delete from DetailQA where iCaseID = " + e.CommandArgument.ToString() + "");
            objQuery.ExecuteSql("delete from OrderDetails where exists (select * from CaseOrder where iOrderID = OrderDetails.iOrderID and iCaseID = " + e.CommandArgument.ToString() + ")");
            objQuery.ExecuteSql("delete from CaseOrder where iCaseID = " + e.CommandArgument.ToString() + "");
            objQuery.ExecuteSql("delete from CaseDocument where iCaseID = " + e.CommandArgument.ToString() + "");
            objQuery.ExecuteSql("delete from CaseNotes where iCaseID = " + e.CommandArgument.ToString() + "");
            objQuery.ExecuteSql("delete from Cases where iCaseID = " + e.CommandArgument.ToString() + "");

            dtpCase_PreRender(new object(), new EventArgs());
        }
    }
    ...
    {
        if (ddlDoctorName.SelectedValue.ToString() != "")
        {
            Users objQuery = new Users();
            string strQuery2 = " select iAppointmentID,isNull(t1.dtAppointmentDate,'')+', '+ isNull(t1.sTimeFrom,'')+', '+isNull(t1.sLocationName,'') AS sAppointmentDetail from (select iAppointmentID ,CONVERT(VARCHAR(12), dtAppointmentDate, 113) as dtAppointmentDate ,sTimeFrom ,(Case when 1=1 then (select  sLocationName from LinkedVenue where iVenueID = objDoc.iVenueID )Else '' END) as sLocationName from  ExpertAppointments objDoc where iAdminCompanyID=" + Session["iAdminCompanyID"].ToString() + " and iContactID =" + ddlDoctorName.SelectedValue.ToString() + "  and cast(dtAppointmentDate as DATE) >= CAST(GETDATE() as Date)) as t1 where not EXISTS(select * from Cases where iAppointmentID=t1.iAppointmentID)";
